...
不使用进程--真隐藏。这种方法一般使用Windows系统动态链接库DLL来避开检测，Windows的所有API函数都是在DLL中实现的，它不包含程序的入口函数，因而不能独立地运行。一般都是由进程在需要的时候加载并调用的，所有在进程和服务列表中并不会出现这些DLL文件。DLL用于木马隐藏的方式：1. 使用攻击者的DLL文件替换掉Windows系统中正常的DLL。2.动态嵌入技术，即讲木马程序的代码嵌入到正在运行的进程中。
...
1.木马监测：检查系统，确定木马是否存在。方法：（1）端口扫描和连接检查。大部分木马的服务端会在系统中监听某个端口，因此通过查看系统上开启了哪些端口就能有效的发现远程控制木马的踪迹（2）检查系统进程。绝大多数的木马在运行时期都会在系统中生成进程，检查系统列表，是一种有效的发现木马踪迹的方法。（3）检查.ini文件、注册表和服务。为了木马自动运行，大部分木马都会把自己登记在开机启动的程序中，这样开机启动就自动加载。少数将木马与特定的可执行文件进行捆绑。木马随着文件自动进行。（4）监视网络通信。一些特殊的木马程序使用ICMP协议进行通讯，被控制端不需要打开任何监听端口，也无须反向链接，没有固定连接。使得netstat或者fport等工具很难发挥作用。检查可疑进程。可以通过嗅探工具，嗅探器软件监视网络通信，比如sniffer或者wireshark来监视网络通信，从网络通信的抓包中发现可疑情况

2.木马的防范

（1）及时修补漏洞、安装budding（2）运行实时监测

5.6缓冲区溢出攻击防御技术

1、源码级保护方法

编写没有漏洞的安全代码是防范缓冲区溢出攻击的最好方法。

2、运行期间保护方法

主要研究如何在程序运行的过程中发现或阻止缓冲区溢出攻击

3、阻止攻击代码执行

<figure><img src="../.gitbook/assets/Screenshot 2026-01-08 at 3.13.02 pm.png" alt=""><figcaption></figcaption></figure>
